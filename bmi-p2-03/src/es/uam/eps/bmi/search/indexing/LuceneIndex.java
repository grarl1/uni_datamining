...
public class LuceneIndex implements Index {
    ...
    public List<Integer> getDocIds() {
        ...
        for (int i = 0; i < reader.numDocs(); ++i) {
            try {
                docIds.add(namesMap.get(reader.document(i).getFieldable("docID").stringValue()));
            } catch (IOException ex) {
                System.err.println("Exception caught while performing an I/O operation: " + ex.getClass().getSimpleName());
                System.err.println(ex.getMessage());
            }
        ...
        try {
            ...
            while (tp.next()) {
                ...
                for (int i = 0; i < tp.freq(); i++) {
                    position_list.add(tp.nextPosition());
                }
                String docID = reader.document(tp.doc()).getFieldable("docID").stringValue();
                lp.add(new Posting(term, namesMap.get(docID), position_list));
            }
        } catch (IOException ex) {
            System.err.println("Exception caught while performing an I/O operation: " + ex.getClass().getSimpleName());
            System.err.println(ex.getMessage());
